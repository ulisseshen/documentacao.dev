---
title: Bem-vindo
description: Catálogo de Documentações em Português Brasileiro
---

<GlobalStyles/>

# Documentação.dev 🇧🇷

**Seu catálogo de documentações técnicas em Português Brasileiro**

Encontre documentação de qualidade em PT-BR para as principais tecnologias de desenvolvimento.

---

## 📚 Documentações Disponíveis

### Flutter

**[FlutterBrasil.dev](https://flutterbrasil.dev)** - Site oficial Flutter em PT-BR
- Framework para criar aplicativos multiplataforma
- Documentação completa traduzida
- Tutoriais e guias em português

**[docs.flutterbrasil.dev](https://docs.flutterbrasil.dev)** - Documentação Flutter
- Referência completa da API
- Guias de desenvolvimento
- Exemplos práticos

### Dart

**[DartBrasil.dev](https://dartbrasil.dev)** - Linguagem Dart e documentação
- Linguagem de programação moderna
- Documentação oficial traduzida
- Recursos de aprendizado

### Angular

**Em breve** 🚧
- Documentação Angular em PT-BR
- Em desenvolvimento

---

## 💝 Apoie Este Projeto

Suas doações ajudam a manter as traduções atualizadas e de qualidade para toda a comunidade brasileira.

<DonationButtons/>

---

## 🤝 Quer Adicionar Sua Documentação?

Se você mantém ou conhece uma documentação técnica em PT-BR de qualidade, [entre em contato](/about) para adicioná-la ao catálogo!

---

**Feito com ❤️ pela comunidade brasileira de desenvolvedores**
